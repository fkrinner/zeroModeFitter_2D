...
#		rhoParameters = [1.27549, .100]
		...
		for sector in sectors:
			...
			if "3--" in sector:
				waveModel[sector] = rhoModel
	elif modelMode == "phaseTest":
		filePath = "/nfs/freenas/tuph/e18/project/compass/analysis/fkrinner/fkrinner/trunk/massDependentFit/scripts/anything/zeroModes/bwAmplitudes_noBF/amp_0mp0pSigmaPiS"
		if not "0-+0+[pi,pi]0++PiS" in sectors:
			raise ValueError("0-+0+[pi,pi]0++PiS is missing")
		param = pc.fixedParameterization(filePath, polynomialDegree  = 0, complexPolynomial = False)
