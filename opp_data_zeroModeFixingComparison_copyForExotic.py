...
from vectorFormFactor import vectorFormFactor
...
#		return
	...
	resultFile  =  "./vectorFormFactorResults_1mp/1mp_vff_t"+str(tBin)+"_m"+str(startBin)+'-'+str(stopBin)+'_'+str(seedint)+".dat"
...
#       # - - - - --- Start the evaluations here --- - - - - #       #
	...
	if doPlots:
		...
		for b in range(startBin, stopBin):
			RV.connectTheoPoints = range(100)
			plotNameBase = "./vectorFormFactor_plots_1mp/1mp1p1mmPiP_<mode>_"+str(b)+"_"+str(tBin)+".pdf"
			RV.writeBinToPdf(b, stdCmd = ["", plotNameBase.replace("<mode>","intens"), [],  plotNameBase.replace("<mode>","argand"), []])
